...
sudo apt install -y breeze-cursor-theme folder-color fonts-emojione fonts-powerline gnome-tweaks hardcode-tray materia-gtk-theme-git materia-kde papirus-icon-theme qt4-qtconfig qt5ct
...
### APT Apps
```bash
sudo add-apt-repository -y ppa:mc3man/bionic-media # Media codecs
sudo add-apt-repository -y ppa:mkusb/ppa # MKUSB
sudo add-apt-repository -y ppa:xuzhen666/gnome-mpv # Celluloid
#sudo add-apt-repository -y ppa:atareao/telegram
sudo apt install -y --no-install-recommends celluloid gthumb kpat mkusb sweeper
...
gsettings set io.github.cellulid-player.Celluloid mpv-options --save-position-on-quit
